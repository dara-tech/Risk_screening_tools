...
TEST_SYS_001,TEST_UUIC_001,Doe,John,Male,1990-05-15,Phnom Penh,OD001,District 1,Commune A,Male,Male,Yes,Yes,Yes,No,No,2,Yes,Yes,Negative,Medium Risk,false,true,false,false,false,false,true,false,false,false,2024-01-15,false,false,15,false,false`
    ...
    const downloadTemplate = () => {
        ...
        const timestamp = Date.now()
        const csvContent = `System ID,UUIC,Family Name,Last Name,Sex,Date of Birth,Province,OD,District,Commune,Donor,NGO,Sex at Birth,Gender Identity,Sexual Health Concerns,Had Sex Past 6 Months,Partner Male,Partner Female,Partner TGW,Number of Sexual Partners,Past 6 Months Practices,HIV Test Past 6 Months,HIV Test Result,Risk Screening Result,Sex with HIV Partner,Sex without Condom,STI Symptoms,Syphilis Positive,Receive Money for Sex,Paid for Sex,Injected Drug Shared Needle,Alcohol Drug Before Sex,Group Sex Chemsex,Currently on PrEP,Last HIV Test Date,Abortion,Forced Sex,Risk Screening Score,None of Above,Ever on PrEP
SYS_${timestamp}_001,UUIC_${timestamp}_001,Doe,John,Male,1990-05-15,Phnom Penh,OD001,District 1,Commune A,USAID,NGO A,Male,Male,Yes,Yes,Yes,No,No,2,Yes,Yes,Negative,Medium Risk,false,true,false,false,false,false,true,false,false,false,2024-01-15,false,false,15,false,false
SYS_${timestamp}_002,UUIC_${timestamp}_002,Smith,Jane,Female,1985-08-22,Battambang,OD002,District 2,Commune B,GFATM,NGO B,Female,Female,Yes,Yes,No,Yes,No,1,Yes,Yes,Negative,Low Risk,false,false,false,false,false,false,false,false,false,false,2024-02-20,false,false,8,false,false
SYS_${timestamp}_003,UUIC_${timestamp}_003,Johnson,Mike,Male,1988-12-10,Siem Reap,OD003,District 3,Commune C,PEPFAR,NGO C,Male,Male,Yes,Yes,Yes,No,No,5,Yes,No,Unknown,High Risk,true,true,true,false,true,false,true,true,true,false,2023-11-30,false,false,45,false,false`
        ...
        showToast({
            ...
            description: 'CSV template with validation examples downloaded successfully',
            variant: 'success'
        })
    }
                                                    ...
                                                    {processedData.data.slice(0, 3).map((record, index) => (
                                                        <div key={index} className="p-2 bg-gray-50 rounded border">
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                                <div><strong>Name:</strong> {record.data.familyName} {record.data.lastName}</div>
                                                                <div><strong>Sex:</strong> {record.data.sex}</div>
                                                                <div><strong>Age:</strong> {record.data.age || 'N/A'}</div>
                                                                <div><strong>Risk Level:</strong> {record.data.riskLevel || 'N/A'}</div>
                                                                <div><strong>Risk Score:</strong> {record.data.riskScore || 'N/A'}</div>
                                                                <div><strong>Location:</strong> {record.data.province}, {record.data.district}</div>
                                                            </div>
